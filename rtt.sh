...
# Sort the IPv6 addresses in reverse order
IFS=$'\n' sorted_addresses=($(sort -r <<<"${ipv6_addresses[*]}"))
unset IFS

additional_address=""
for ((i = 0; i <= ${#sorted_addresses[@]}; i++)); do
    # Extract the last part of the current IPv6 address
    current_last_part=$(echo "${sorted_addresses[i]##*:}")

    # Increment the last part to generate the modified address
    modified_last_part_hex=$(printf '%04x' "$((16#$current_last_part + 1))")

    # Generate the modified address
